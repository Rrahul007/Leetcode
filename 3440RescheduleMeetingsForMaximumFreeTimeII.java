class Solution {
    public int maxFreeTime(int eventTime, int[] startTime, int[] endTime) {
        ...
        for(int i = 0; i < n; ++i)
        {
            final int currentMeetingTime = endTime[i] - startTime[i];
            final int adjecentGapsSum = gaps[i] + gaps[i + 1];
            final boolean canMoveMeeting = currentMeetingTime <= Math.max( i > 0 ? maxLeft[i - 1] : 0, i + 2 < n + 1 ? maxRight[i + 2] : 0);
            ans = Math.max(ans, adjecentGapsSum + (canMoveMeeting ? currentMeetingTime : 0));
        }
        return ans;
    }
